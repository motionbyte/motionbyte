...
import { PORTFOLIO_WORDMARK_PNG } from './portfolioWordmarkAssets'

function PortfolioLights() {
  return (
    <>
...
function PortfolioParticlesLayer({ assetUrl }: { assetUrl: string }) {
  ...
  const particleData = useMemo(() => {
    ...
    if (srcW < 2 || srcH < 2) {
      console.warn('[PortfolioWordmark] Image not ready or invalid size', srcW, srcH)
    }
    const collect = PORTFOLIO_WORDMARK_PNG.treatAsOpaqueBitmap
      ? {
          maxDim: 520,
          sampleStep: 1,
    ...
    if (import.meta.env.DEV && data.count === 0) {
      console.warn('[PortfolioWordmark] 0 particles — check portfolio.png / sampling')
    }
    return data
  }, [texture])
...
export function PortfolioWordmarkParticles() {
  ...
  const assetUrl = PORTFOLIO_WORDMARK_PNG.url + devBust

  return (
    <div ref={rootRef} className="portfolio-wordmark-stack">
      <Canvas
        className="portfolio-wordmark-canvas"
      ...
      >
        ...
        <Suspense fallback={null}>
          <PortfolioParticlesLayer assetUrl={assetUrl} />
        </Suspense>
      </Canvas>
    </div>
  )
